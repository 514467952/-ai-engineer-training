...
try:
    ...
    raise ImportError("pip install paddlepaddle==2.6.1 -i https://pypi.tuna.tsinghua.edu.cn/simple")
...
class ImageOCRReader(BaseReader):
    ...
    def load_data(self, file: Union[str, Path, List[Union[str, Path]]]) -> List[Document]:
        ...
        for image_path in files:
            ...
            # 使用 PaddleOCR 提取文本
            result = self._ocr.ocr(image_path_str)
...
def setup_environment():
    ...
    if not api_key:
        raise ValueError("没有apikey，需要输入环境变量DASHSCOPE_API_KEY")
...
# 在根目录可以通过python -m ocr_research.main 运行
    ...
    try:
        ...
        print(f"Created imagesxxxx: {image_files}")
    ...
    documents = reader.load_data("data/ocr_images/general_ocr_002.png")
